...
function Project({ title, image, description, techstack, previewLink, githubLink, contributions,  }) {
      ...
      <article className="rounded-xl mt-10 overflow-hidden shadow-xl shadow-slate-300 dark:shadow-slate-900 border-4 group border-blue-800 hover:bg-blue-200">
        <div className="relative w-full h-60 overflow-hidden bg-blue-200 border-b-4 border-b-blue-500">
          ...
          <div className="absolute z-20 w-full h-full group-hover:flex hidden items-center justify-center">
            <div 
              className="w-32 h-10 rounded-md border-blue-800 border-2 group-hover:flex items-center justify-center hidden bg-blue-400 text-black hover:cursor-pointer hover:bg-blue-500"
        ...
        <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-lg text-left">
            <h2 className="text-2xl mb-4">Contributions for Project "{title}"</h2>
            <p className="mb-4">Below is what I achieved in this project: </p>
            
            {contributions?.length > 0 && (
                <ol className="list-decimal ml-4">
                  {contributions.map(contribution => (
                    <li className="mt-2">{contribution}</li>
                  ))}
                </ol>
              )
            }
